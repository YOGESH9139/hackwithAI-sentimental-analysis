...
// Component to fetch live price for a portfolio position to calculate value
function PortfolioValuation({
  positions,
  onUpdate
}: {
  positions: any[],
  onUpdate: (val: number, change: number) => void
}) {
  const [values, setValues] = useState<Record<string, { price: number, prevClose: number }>>({})
  ...
  useEffect(() => {
    ...
    const fetchPrices = async () => {
      ...
      for (const pos of positions) {
        try {
          // In a real app we'd use a bulk quote API to avoid multiple requests
          const res = await fetch(`/api/stock/quote?symbol=${encodeURIComponent(pos.symbol)}`)
          if (res.ok) {
            const data = await res.json()
            totalP += (data.c * pos.shares)
            totalC += ((data.c - data.pc) * pos.shares)
          }
        } catch (e) {
          console.error(e)
        }
      }
      if (isMounted) onUpdate(totalP, totalC)
    }
      ...
      <div className="flex items-center justify-between p-4 rounded-xl bg-white/5 hover:bg-white/10 transition-all border border-white/5 hover:border-primary/20">
        ...
        <div className="text-right min-w-[80px]">
          {isLoading ? (
            ...
            <>
              <p className="font-mono text-foreground">{formatCurrency(data.c, symbol)}</p>
...
export default function DashboardPage() {
  ...
  const dayChangePercent = stockValue > 0 ? (dayChange / (stockValue - dayChange)) * 100 : 0
    ...
    <div className="p-6">
      ...
      <div className="glass-card rounded-2xl p-5 hover:border-primary/20 transition-all">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className={`h-8 w-8 rounded-lg flex items-center justify-center ${dayChange >= 0 ? 'bg-[#00D094]/10' : 'bg-[#ef4444]/10'
            }`}>
            {dayChange >= 0 ? (
        ...
        <div className={`text-2xl font-bold font-mono ${dayChange >= 0 ? 'text-[#00D094]' : 'text-[#ef4444]'
          }`}>
          {dayChange >= 0 ? '+' : ''}{formatCurrency(dayChange)}
        </div>
        <p className={`text-xs mt-1 ${dayChangePercent >= 0 ? 'text-[#00D094]' : 'text-[#ef4444]'
          }`}>
